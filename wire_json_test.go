...
func TestJSONUnmarshalRejectsUnknownFields(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "instant legacy epoch",
			input:  `{"kind":"instant","iso":"1970-01-01T00:00:00Z","epoch_ms":0}`,
		...
		{
			name:   "date legacy calendar",
			input:  `{"kind":"date","value":"2026-03-27","calendar":"iso8601"}`,
			decode: func(b []byte) error { var v Date; return json.Unmarshal(b, &v) },
		},
		{
			name:   "local datetime legacy calendar",
			input:  `{"kind":"local_datetime","value":"2026-03-27T13:30:45","calendar":"iso8601"}`,
			decode: func(b []byte) error { var v LocalDateTime; return json.Unmarshal(b, &v) },
		},
		{
			name:   "date extra field",
			input:  `{"kind":"date","value":"2026-03-27","weekday":"Friday"}`,
			decode: func(b []byte) error { var v Date; return json.Unmarshal(b, &v) },
		},
		{
			name:   "time legacy precision",
			input:  `{"kind":"time","value":"13:30:45","precision":"second"}`,
			decode: func(b []byte) error { var v Time; return json.Unmarshal(b, &v) },
		},
		{
			name:   "datetime legacy local value",
...
func TestJSONUnmarshalRejectsMissingRequiredFields(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:  `{"kind":"datetime","zone":"Asia/Tokyo"}`,
			decode: func(b []byte) error { var v DateTime; return json.Unmarshal(b, &v) },
		},
		{
			name:   "duration iso",
